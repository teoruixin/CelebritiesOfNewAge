...
function countYoutubers(data, target) {
  var youtuberCount = {}
  for (var row of data) {
    if (row[target] == "nan") {
      continue;
    ...
    if (row[target] in youtuberCount) {
      ...
      youtuberCount[row[target]] = { "country": row.Country, "value": 1 }
    }
  }

  var output = [];
  for (var country_dict in youtuberCount) {
    output.push(youtuberCount[country_dict]);
  }
  return output;
}
// Make chart.
function treemap_create(input_data) {
  ...
  const countries = [...new Set(input_data.map(d => d.Country))].filter(d => d != "nan")
  // console.log(countries)

  var countriesYoutubers = countYoutubers(input_data, "Country")
  // for (var i = 0; i < countries.length; i++) {
  //   // console.log(countYoutubers(countries[i]))
  //   countriesYoutubers[i] = countYoutubers(countries[i], input_data)
  // }
